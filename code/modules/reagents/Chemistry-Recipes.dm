	...
	if(holder && holder.my_atom)
		if (chemical_mob_spawn_meancritters.len <= 0 || chemical_mob_spawn_nicecritters.len <= 0)
			for (var/T in typesof(/mob/living/simple_animal))
				...
				if(istype(T,/mob/living/simple_animal/hostile/headcrab) || istype(T,/mob/living/simple_animal/hostile/boss))
					continue //Prevents headslugs from the golden slimes and bosses

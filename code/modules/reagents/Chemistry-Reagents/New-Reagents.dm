...
datum/reagent/meth
	...
	color = "#3333FF" // rgb: 205,133,63
	addiction_rate = 0.2 
datum/reagent/meth/on_mob_life(var/mob/living/M as mob)
	var/mob/living/carbon/human/H = M
	H.addicted_to.add_reagent("meth", 0.2) //Slowly add addiction
	var/datum/reagent/R = H.addicted_to.has_reagent("meth")
	if(R)
		R.data = R.data-- //Slowly reduce addiction stage to prevent toxins as soon as you stop smoking when addicted.
		if(R.data < 0)
			R.data = 0
...
datum/reagent/meth/on_mob_addicted(var/mob/living/M as mob)//copied from nicotine , this is annoying to code so I don't care. Update if you want.
	var/mob/living/carbon/human/H = M
	//Restructured the IF check - possibly fixed the runtime error.
	if(istype(H)) //Addiction slowly builds up. We don't call ..() because we don't want to remove it yet.
		var/obj/item/clothing/mask/cigarette/cig = H.wear_mask
	...
	switch(data)
		...
		if(15 to 30)
			if(prob(4))
				M.emote("cough")
			if(prob(7))
				var/msg = pick("You feel pretty bad.", "You feel like you could throw up!", "You have a headache.")
				M << "<span class='notice'>[msg]</span>"
				H.adjustStaminaLoss(-5)
		if(30 to 60)
			if(prob(5))
				M.emote("twitch")
				M.adjustToxLoss(3) //Not feeling so well now are you?
				H.Stun(1)
			if(prob(7))
				var/msg = pick("You feel pretty bad.", "You feel like shit.", "Your inside burn.", "You have a headache.", "You feel nauseous.")
				M << "<span class='notice'>[msg]</span>"
		...
		if(60 to INFINITY)
			if(prob(10))
				M.emote("twitch") 
				M.adjustToxLoss(5) //Toxins intensify
				H.Stun(4)
			if(prob(7))
				var/msg = pick("You feel nauseous.", "You feel intoxicated.", "Goddamn, you feel terrible!", "You feel like your meth addiction is spiraling out of control.")
				M << "<span class='warning'>[msg]</span>"
			if(prob(2))
				M.visible_message("<span class='danger'>[src] clutches his chest!</span>")
				M.Stun(rand(5,10))
				M.apply_damage(rand(8,15), BRUTE, "chest")
				if(prob(5))
					M.emote("scream")

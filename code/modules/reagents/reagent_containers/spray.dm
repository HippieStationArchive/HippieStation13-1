...
/obj/item/weapon/reagent_containers/spray/afterattack(atom/A as mob|obj, mob/user)
	...
	if(reagents.has_reagent("sacid"))
		message_admins("[key_name_admin(user)] fired sulfuric acid from \a [src] at (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>[get_area(src)] ([T.x], [T.y], [T.z])</a>).")

	...
	for(var/turf/T in targets)
		spawn(0)
			...
			for(var/obj/machinery/mach in T.contents)
				if(istype(mach, /obj/machinery/dominator) || istype(mach, /obj/machinery/power/apc) || istype(mach, /obj/machinery/power/smes)) //Doesn't work on dominators and APCs, to prevent kekkery
					continue
				if(prob(30))

...
/datum/admin_ticket/proc/toggle_resolved()
	...
	if(resolved)
		...
		var/DBQuery/query = dbcon.NewQuery("SELECT id FROM [format_table_name("admin_tickets")] WHERE round_id = [hippie_round_number] AND ticket_id = [ticket_id]")
		query.Execute()
		if(!query.NextRow())
			var/content = ""
			for(var/datum/ticket_log/line in log)
				content += "[line.user][line.user_admin ? " A" : ""]: [line.text]\n"
			var/DBQuery/insert = dbcon.NewQuery("INSERT INTO [format_table_name("admin_tickets")] (round_id, ticket_id, ckey, a_ckey, content) VALUES ([hippie_round_number], [ticket_id], '[owner_ckey]', '[get_client(handling_admin)]', '[content]')")
			insert.Execute()

...
/obj/machinery/cerealmaker/attackby(obj/item/I, mob/user)
	...
	var/icon/cerealbox_icon = icon(initial(S.icon), initial(S.icon_state), , 1)
	cerealbox_icon.Blend("#fff", ICON_ADD) 			//fills the icon_state with white (except where it's transparent)
	cerealbox_icon.Blend(icon(I.icon, I.icon_state), ICON_MULTIPLY) //adds item image and the remaining white areas become transparant

...
/client/New(TopicData)
	...
	if (isnum(player_age) && player_age == -1) //first connection
		if(config.proxyban) // proxyban's enabled
			var/danger = proxycheck()
			if(danger >= config.proxybanlimit)
				...
				switch(danger)
					if(-1)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -1, IP-less client. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -1, IP-less client ( [address] )")
					if(-2)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -2, IP invalid. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -2, IP invalid ( [address] )")
					if(-3)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -3, private address detected. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -3, private address detected. ( [address] )")
					if(-4)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -4, ProxyChecker not working, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -4, ProxyChecker not working, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						if(!config.panic_bunker)
							panicbunker()
					if(-5)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -5, ProxyChecker stopped working! Server banned from the system. Fix immediately, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -5, ProxyChecker stopped working! Server banned from the system. Fix immediately, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						if(!config.panic_bunker)
							panicbunker()
					if(-6)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -6, ProxyChecker stopped working! PROXYBANEMAIL config option is invalid. Fix immediately, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -6, ProxyChecker stopped working! PROXYBANEMAIL config option is invalid. Fix immediately, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						if(!config.panic_bunker)
							panicbunker()
					if(-7)
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -7, ProxyChecker stopped working! Number of queries possible per minute exceeded, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -7, ProxyChecker stopped working! Server banned from the system. Number of queries possible per minute exceeded, [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						if(!config.panic_bunker)
							panicbunker()
					else
						message_admins("<span class='adminnotice'>Failed Login: [key] - ProxyBan error code -8, ProxyChecker stopped working! HTTP error code [danger], [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						log_access("Failed Login: [key] - ProxyBan errror code -8, ProxyChecker stopped working! HTTP error code [danger], [config.panic_bunker ? "panic bunker already active" : "activating panic bunker"]. ( [address] )")
						if(!config.panic_bunker)
							panicbunker()
				del(src)
				return 0
...
/client/proc/proxycheck()
	var/list/httpstuff = world.Export("http://check.getipintel.net/check.php?ip=[address]&contact=[config.proxybanemail]&flags=b")
	if(!httpstuff)

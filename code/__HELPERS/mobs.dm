...
/proc/add_logs(mob/user, mob/target, what_done, object=null, addition=null)
	...
	if(user && ismob(user))
		user.attack_log += text("\[[time_stamp()]\] <font color='red'>Has [what_done] [target ? "[target.name][(ismob(target) && target.ckey) ? "([target.ckey])" : ""]" : "NON-EXISTANT SUBJECT"][object ? " with [object]" : " "] @ [target ? "[target.x],[target.y],[target.z]" : ""][addition][newhealthtxt]</font>")
	if(target && ismob(target))
		target.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been [what_done] by [user ? "[user.name][(ismob(user) && user.ckey) ? "([user.ckey])" : ""]" : "NON-EXISTANT SUBJECT"][object ? " with [object]" : " "] @ [user.x],[user.y],[user.z] [addition][newhealthtxt]</font>")

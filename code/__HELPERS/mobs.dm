...
/proc/random_features()
	...
	if(!tails_list_shark.len)
		init_sprite_accessory_subtypes(/datum/sprite_accessory/tails/shark, tails_list_shark)
	if(!snouts_list.len)
	...
	return(list("mcolor" = pick("FFFFFF","7F7F7F", "7FFF7F", "7F7FFF", "FF7F7F", "7FFFFF", "FF7FFF", "FFFF7F"), "tail_lizard" = pick(tails_list_lizard), "tail_human" = "None",  "tail_shark" = pick(tails_list_shark), "snout" = pick(snouts_list), "horns" = pick(horns_list), "ears" = "None", "frills" = pick(frills_list), "spines" = pick(spines_list), "body_markings" = pick(body_markings_list), "wing" = pick(wing_list)))

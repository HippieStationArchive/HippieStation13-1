...
/obj/spacepod
	...
	var/datum/effect_system/trail_follow/space_trail/ion_trail
...
/obj/spacepod/New()
	...
	add_cabin()
	add_airtank()
	ion_trail = new /datum/effect_system/trail_follow/space_trail()
...
/obj/spacepod/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/device/spacepod_equipment))
		if(!hatch_open)
			return ..()
		if(!equipment_system)
			user << "<span class='warning'>The pod has no equipment datum, yell at pomf, or TD for failing to port this correctly</span>"
			return
		if(istype(W, /obj/item/device/spacepod_equipment/weaponry))
			var/obj/item/device/spacepod_equipment/weaponry/WEAP = W
...
/obj/spacepod/attack_hand(mob/user as mob)
	...
	if(!equipment_system || !istype(equipment_system))
		user << "<span class='warning'>The pod has no equipment datum, or is the wrong type, yell at pomf and/or TD</span>"
		return
...
/obj/spacepod/verb/toggle_internal_tank()
	...
	occupant << "<span class='notice'>Now taking air from [use_internal_tank?"internal airtank":"environment"].</span>"
	return
...
*/
	if(usr.buckled_mob)
		usr << "<span class='warning'>You can't enter [src] with [usr.buckled_mob] attached to you!</span>"
		return
...
/obj/spacepod/proc/Process_SpacePodmove(var/check_drift = 0, mob/user)
	var/dense_object = 0
	if(!user)
...
/obj/spacepod/relaymove(mob/user, direction)
	...
	if(battery && battery.charge >= 3 && health)
		...
		if(!battery)
			...
			user << "<span class='warning'>Unknown error has occurred, yell at Pomf, or at TD because he failed to port this correctly.</span>"
		return 0
...
/obj/spacepod/cadillac
	name = "space cadillac"
	desc = "A space cadillac"
	icon = 'icons/64x64/cadillac.dmi'
	icon_state = "cadillac"

// Pod trails
/datum/effect_system/trail_follow/space_trail
	var/turf/oldloc // secondary ion trail loc
	var/turf/currloc

/datum/effect_system/trail_follow/space_trail/start()
	if(!src.on)
		src.on = 1
		src.processing = 1
	if(src.processing)
		...
		spawn(0)
			...
			if(currloc != T)
				...
				if(istype(T, /turf/space))
					var/obj/effect/particle_effect/ion_trails/I = PoolOrNew(/obj/effect/particle_effect/ion_trails,oldposition)
					var/obj/effect/particle_effect/ion_trails/II = PoolOrNew(/obj/effect/particle_effect/ion_trails,oldloc)
					//src.oldposition = T

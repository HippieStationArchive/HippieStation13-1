...
/obj/machinery/door_timer/initialize()
	for(var/obj/machinery/door/poddoor/glass/G in spiral_range(20, src))
		if (G.id == src.id)
			targets += G

	for(var/obj/machinery/disposal/trapdoor/T in spiral_range(20, src))
		if (T.id == src.id)
			targets += T

	for(var/obj/machinery/flasher/F in spiral_range(20, src))
		if(F.id == src.id)
			targets += F

	for(var/obj/structure/closet/secure_closet/brig/C in spiral_range(20, src))
		if(C.id == src.id)
			targets += C

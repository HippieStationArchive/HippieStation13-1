...
/obj/machinery/emagproof_button/New(loc, ndir = 0, built = 0)
	..()
	if(built)
		dir = ndir
		pixel_x = (dir & 3)? 0 : (dir == 4 ? -24 : 24)
...
/obj/machinery/emagproof_button/update_icon()
	overlays.Cut()
	if(panel_open)
		icon_state = "button-open"
		if(device)
...
/obj/machinery/emagproof_button/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/device/detective_scanner))
		return
	return src.attack_hand(user)

/obj/machinery/emagproof_button/attack_hand(mob/user)
	src.add_fingerprint(user)
	if(panel_open)
		if(device || board)
			if(device)
...
/obj/machinery/emagproof_button/door
	name = "door button"
	desc = "A door remote control switch."
	var/normaldoorcontrol = 0
	var/specialfunctions = OPEN // Bitflag, see assembly file

/obj/machinery/emagproof_button/door/New(loc, ndir = 0, built = 0)
	if(id && !built && !device)
		if(normaldoorcontrol)
			var/obj/item/device/assembly/control/airlock/A = new(src)
			device = A

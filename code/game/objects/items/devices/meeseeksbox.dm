...
/obj/item/device/meeseeks_box/attack_self(mob/user)
	if(summoned && !meeseeks)	//happens if the meeseeks is destroyed without being desummoned
		...
		for(var/turf/T in orange(80))
			...
			turfs += T
		visible_message("<span class='danger'> The Meeseeks box colllapses in on itself and warps space around it!")
	...
	if(!meeseeks && (summon_time ) < world.time)
		...
		var/list/candidates = pollCandidates("Do you wish to be a Mr. Meeseeks and fulfill a task?", "pAI", null)
		shuffle(candidates)
		if(candidates.len)
			var/mob/dead/observer/C = pick(candidates)
			var/mob/living/carbon/human/M = new /mob/living/carbon/human
			M.hardset_dna(null, null, "Mr. Meeseeks ([rand(1, 1000)])", null, /datum/species/golem/meeseeks, null)

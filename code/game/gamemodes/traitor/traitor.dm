...
/datum/game_mode/traitor
	...
	restricted_jobs = list("Security Officer", "Warden", "Detective", "Cyborg", "AI", "Captain", "Head of Personnel", "Chief Medical Officer", "Research Director", "Chief Engineer", "Head of Security")//They are part of the AI if he is traitor so are they, they use to get double chances
		...
		for(var/i = objective_count, i < config.traitor_objectives_amount, i++)
			if(prob(50))
				if(active_ais.len && prob(100/joined_player_list.len))
					...
					kill_objective.find_target()
					traitor.objectives += kill_objective
					has_to_kill = kill_objective.target
				else
